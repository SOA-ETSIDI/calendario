...

parseCalendar <- function(cal)
{
    ## Días festivos
    festivos <- cal[Formato == 'fiesta']
    ## Días sueltos
    ...
    ## GRADO
    calGrado <- cal[Tipo %in% c("ETSIDI", "Grado")]
    ...
    calMaster <- cal[Tipo %in% c("ETSIDI", "Master")]
    ...
    idxCambio <- grep("Horario de", cal$Descripcion)
    dCambios <- cal[idxCambio, Inicio]
    cambios <- sub("Horario de ", "", cal[idxCambio, Descripcion])
